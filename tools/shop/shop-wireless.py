#!/usr/bin/env python3
"""Add the wireless lamps (the power node and its two fixtures) to the survival
shop.

Same shape as shop-medieval.py: a text-level merge where untouched entries
...
anything is changed. No new categories: the node is network hardware and goes
in `industrial` with the gauges and the poles, the fixtures are lights and go
in `lighting` with the lanterns.
...
# ---- the wave -------------------------------------------------------------
#
# The node is the expensive thing: one lights a whole room and takes the same
# Simple Upgrades every Machines block takes. A fixture is priced as a light,
# beside the lanterns and the wired panels it replaces, a little over them
# because it needs no wire run to it, and up the ladder with what it draws.

ITEMS = [
	('power', 'industrial', 45.0, 'wireless_power_node'),
	('light', 'lighting', 6.0, 'wireless_bulb'),
	('light', 'lighting', 8.0, 'wireless_light_panel'),
	# the full block draws twice the panel and is priced over it by half: it is
	# a building block as much as a fitting, and a floor takes a lot of them
	('light', 'lighting', 12.0, 'wireless_light_block'),
	# the bollard, and the switch that toggles a channel: the switch is a
	# control, so it lives with the other network hardware
	('light', 'lighting', 10.0, 'wireless_light_pole'),
	('power', 'industrial', 8.0, 'wireless_light_switch'),
	('power', 'industrial', 8.0, 'wireless_light_button'),
]
