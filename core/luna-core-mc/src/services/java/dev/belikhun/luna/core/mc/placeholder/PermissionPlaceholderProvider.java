...
final class PermissionPlaceholderProvider implements ServerPlaceholderProvider {
	...
	private String resolveLuckPermsValue(BuiltInPlaceholderService support, ServerPlayer player, String normalizedParams) {
	...
	private String resolveVaultValue(BuiltInPlaceholderService support, ServerPlayer player, String normalizedParams) {
		return switch (normalizedParams) {
			case "prefix" -> support.safe(permissionService.getPlayerPrefix(player.getUUID()));
			case "suffix" -> support.safe(permissionService.getPlayerSuffix(player.getUUID()));
