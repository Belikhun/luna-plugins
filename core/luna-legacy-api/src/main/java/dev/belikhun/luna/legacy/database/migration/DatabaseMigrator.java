...
import dev.belikhun.luna.legacy.exception.MigrationException;
...
public final class DatabaseMigrator {
	...
	private Map<String, Set<Integer>> appliedVersions() {
		...
		for (Map<String, Object> row : database.query("SELECT namespace, version FROM " + MIGRATION_TABLE, Collections.<Object>emptyList())) {
			...
			if (rawVersion instanceof Number) {
				Number number = (Number) rawVersion;
	...
	private void applyMigration(DatabaseMigration migration) {
		...
		database.update(
			...
			Arrays.<Object>asList(namespace, migration.version(), migration.name(), Instant.now().toEpochMilli())
		);
	...
	private boolean hasNamespace(DatabaseMigration migration) {
		String namespace = normalizeNamespace(migration.namespace());
		return !Strings.isBlank(namespace);
	}
