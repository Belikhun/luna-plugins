...
public final class LunaLore {
	...
	public static List<String> wrapLoreLine(String line, int maxWordsPerLine) {
		...
		String continuationIndent = Formatters.repeated(" ", leadingSpaces + 1);
		...
		if (leadingSpaces > 0) {
			current.append(Formatters.repeated(" ", leadingSpaces));
		}
		int words = 0;
		...
		return wrapped.isEmpty() ? Arrays.asList(normalized) : wrapped;
	}
	...
	private static final class OpenTag {
		...
		private OpenTag(String name, String openTag) {
			this.name = name;
