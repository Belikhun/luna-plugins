...
public final class CommandStrings {
	...
	public static String syntaxRaw(String rawCommand, Segment... segments) {
		if (rawCommand == null || Strings.isBlank(rawCommand)) {
			return "";
		}
	...
	private static String render(Segment segment) {
		switch (segment.kind()) {
			case LITERAL:
				return "<color:" + LunaPalette.TEAL_500 + ">" + segment.name() + "</color>";

			case REQUIRED:
				return "<color:" + typeColor(segment.type()) + ">\\<</color>" + renderParamName(segment.name(), segment.type()) + "<color:" + typeColor(segment.type()) + ">></color>";
	...
	private static String plainSegment(Segment segment) {
		// every arm is the same today; kept as a method so a kind that renders
		// differently in plain text has somewhere to go
		return segment.name();
	}
	...
	private static String normalizeType(String type) {
		...
		return Strings.isBlank(normalized) ? "text" : normalized;
	}
	...
	/** A record on the modern line; the accessors are named to match. */
	public static final class Segment {
		private final SegmentKind kind;
		private final String name;
		private final String type;

		public Segment(SegmentKind kind, String name, String type) {
			this.kind = kind;
			this.name = name;
			this.type = type;
		}
