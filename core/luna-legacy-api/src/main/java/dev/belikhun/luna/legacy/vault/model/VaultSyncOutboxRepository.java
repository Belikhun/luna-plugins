...
import dev.belikhun.luna.legacy.model.ModelRepository;
...
public final class VaultSyncOutboxRepository {
	...
	public void enqueue(UUID operationId, UUID actorId, String actorName, UUID playerId, String playerName, long balanceMinor, String source, String details, long nowEpochMillis) {
		...
		repository.newModel()
			...
			.set("source_plugin", Strings.isBlank(source) ? "backend-sync" : source)
			.set("details", details)
	...
	public List<PendingSyncOperation> due(long nowEpochMillis, int limit) {
		...
		List<Map<String, Object>> rows = database.query(
			...
			Arrays.<Object>asList(STATE_PENDING, nowEpochMillis, safeLimit)
		);
	...
	public void markAcked(UUID operationId, long nowEpochMillis) {
		...
		database.update(
			...
			Arrays.<Object>asList(STATE_ACKED, nowEpochMillis, nowEpochMillis, operationId.toString())
		);
	}
	...
	public void markRetry(UUID operationId, int attemptCount, long nextRetryAt, String error, long nowEpochMillis) {
		...
		database.update(
			...
			Arrays.<Object>asList(STATE_PENDING, Math.max(1, attemptCount), Math.max(nowEpochMillis, nextRetryAt), trimError(error), nowEpochMillis, operationId.toString())
		);
	}
	...
	public void markInFlight(UUID operationId, long lockUntilEpochMillis, long nowEpochMillis) {
		...
		database.update(
			...
			Arrays.<Object>asList(Math.max(nowEpochMillis, lockUntilEpochMillis), nowEpochMillis, operationId.toString(), STATE_PENDING)
		);
	}
	...
	public void markFailed(UUID operationId, int attemptCount, String error, long nowEpochMillis) {
		...
		database.update(
			...
			Arrays.<Object>asList(STATE_FAILED, Math.max(1, attemptCount), trimError(error), nowEpochMillis, operationId.toString())
		);
	}
	...
	public int cleanupAckedOlderThan(long beforeEpochMillis, int limit) {
		...
		List<Map<String, Object>> rows = database.query(
			...
			Arrays.<Object>asList(STATE_ACKED, beforeEpochMillis, safeLimit)
		);
		if (rows.isEmpty()) {
			return 0;
		}
		...
		for (Map<String, Object> row : rows) {
			...
			deleted += database.update("DELETE FROM vault_sync_outbox WHERE operation_id = ?", Arrays.<Object>asList(operationId));
		}
		return deleted;
	}
	...
	public PendingSyncOperation(UUID operationId, UUID actorId, String actorName, UUID playerId, String playerName, long balanceMinor, String source, String details, int attemptCount, long nextRetryAt, long createdAt, long updatedAt) {
