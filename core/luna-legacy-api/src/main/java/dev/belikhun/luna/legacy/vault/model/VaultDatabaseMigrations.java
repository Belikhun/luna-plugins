...
import dev.belikhun.luna.legacy.database.migration.DatabaseMigration;
	...
	public static void register(DatabaseMigrator migrator) {
		migrator.register(new DatabaseMigration() {
			...
			public void migrate(Database database) {
				...
				database.update("CREATE INDEX IF NOT EXISTS vault_transactions_sender_idx ON vault_transactions (sender_uuid, completed_at)", Collections.emptyList());
				database.update("CREATE INDEX IF NOT EXISTS vault_transactions_receiver_idx ON vault_transactions (receiver_uuid, completed_at)", Collections.emptyList());
			}
		});
		...
		migrator.register(new DatabaseMigration() {
			...
			public void migrate(Database database) {
				...
				database.update("CREATE INDEX IF NOT EXISTS vault_sync_outbox_state_retry_idx ON vault_sync_outbox (state, next_retry_at)", Collections.emptyList());
				database.update("CREATE INDEX IF NOT EXISTS vault_sync_outbox_player_idx ON vault_sync_outbox (player_uuid, created_at)", Collections.emptyList());
				database.update("CREATE INDEX IF NOT EXISTS vault_sync_outbox_acked_idx ON vault_sync_outbox (state, acked_at)", Collections.emptyList());
			}
		});
	}
}
