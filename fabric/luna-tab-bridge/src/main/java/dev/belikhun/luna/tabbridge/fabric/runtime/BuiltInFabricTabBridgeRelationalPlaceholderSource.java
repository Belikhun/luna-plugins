...
public final class BuiltInFabricTabBridgeRelationalPlaceholderSource implements FabricTabBridgeRelationalPlaceholderSource {
	...
	public BuiltInFabricTabBridgeRelationalPlaceholderSource(MinecraftServer server, PermissionService permissionService) {
		this.server = server;
	...
	public Map<String, Map<String, String>> resolve(ServerPlayer viewer) {
		...
		for (ServerPlayer target : PlayerLookup.all(server)) {
			...
			String targetName = safe(target.getScoreboardName());
