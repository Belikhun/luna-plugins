...
public final class Compat {
	...
	public static void registerPayloads() {
		PayloadTypeRegistry.serverboundPlay().register(TvPayload.HELLO, TvPayload.codec(TvPayload.HELLO));
		PayloadTypeRegistry.serverboundPlay().register(TvPayload.INPUT, TvPayload.codec(TvPayload.INPUT));
		PayloadTypeRegistry.clientboundPlay().register(TvPayload.SCREENS, TvPayload.codec(TvPayload.SCREENS));
	}


	/**
	 * Nothing to register: the block outline is suppressed by a mixin here.
	 *
	 * This game line has no world render events at all, so there is no
	 * BEFORE_BLOCK_OUTLINE to say no to. LevelRendererMixin cancels the submit
	 * instead, which is the same decision made one layer down.
	 */
	...
	public static void installHud() {
		HudElementRegistry.addLast(
			Identifier.fromNamespaceAndPath("lunatvclient", "input_hint"),
		...
		HudElementRegistry.replaceElement(VanillaHudElements.CROSSHAIR, original -> (graphics, tick) -> {
			if (!HudCursor.keyboardCaptured()) {
				original.extractRenderState(graphics, tick);
