...
public final class LunaVaultEconomyService implements ShopEconomyService {
	...
	@Override
	public CompletableFuture<Double> balanceAsync(ServerPlayer player) {
		if (player == null) {
			return CompletableFuture.completedFuture(Double.valueOf(0D));
		}

		return vaultApi.balance(player.getUUID(), player.getName().getString())
	...
	@Override
	public CompletableFuture<Boolean> hasAsync(ServerPlayer player, double amount) {
		if (player == null || amount < 0D) {
			return CompletableFuture.completedFuture(Boolean.FALSE);
		}

		return vaultApi.has(player.getUUID(), player.getName().getString(), VaultMoney.fromDouble(amount))
	...
	@Override
	public CompletableFuture<Boolean> withdrawAsync(ServerPlayer player, double amount) {
		if (player == null || amount <= 0D) {
			return CompletableFuture.completedFuture(Boolean.FALSE);
		}
	...
	@Override
	public CompletableFuture<Boolean> depositAsync(ServerPlayer player, double amount) {
		if (player == null || amount <= 0D) {
			return CompletableFuture.completedFuture(Boolean.FALSE);
		}
	 ...
	 *
	 * There is no timeout here and there must not be one. The vault gateway already
	 * fails a request it has given up on, so this cannot hang, and a second shorter
	 * deadline would recreate exactly the abandoned-call problem the blocking path
	 * needs {@link #abandon} to repair.
	 */
	...
	private <T> T await(CompletableFuture<T> future, T fallback) {
		...
		T result = FutureUtils.await(future, timeoutMillis + READ_GRACE_MILLIS, fallback, server.isSameThread());
