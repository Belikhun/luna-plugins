...
/**
 * Turning a world id back into a dimension key, as the 1.20-1.21 line spells it.
 *
 * The registry key type itself is stable; only the identifier class it is built
 * from was renamed, so this is the same one-method split {@link ItemLookup} is.
...
public final class DimensionKeys {
	...
	public static ResourceKey<Level> parse(String worldId) {
		...
		try {
			return ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse(worldId.trim()));
