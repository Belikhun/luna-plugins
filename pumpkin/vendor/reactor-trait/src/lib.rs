...
// luna's addition: upstream has only unix and windows arms, so this crate does
// not build for wasm32-wasip2 at all. Nothing in luna calls it - it arrives as a
// transitive dependency of amq-protocol-tcp, which amq-protocol pulls in whether
// or not the TCP half is used - but an unused dependency still has to compile.
// wasip2 has no fd type to borrow, so the trait is Read + Write and no more.
#[cfg(target_os = "wasi")]
mod sys {
    use std::io::{Read, Write};

    pub trait IO: Read + Write {}
    impl<H: Read + Write> IO for H {}
}
