...
public final class PluginMessagingBus implements PluginMessageBus<ServerPlayer, ServerPlayer> {
	private volatile PayloadFallback payloadFallback;
	...
	public PluginMessagingBus(LunaLogger logger, BackendIdentity backendIdentity, boolean loggingEnabled) {
	 ...
	 * class that a loader has to supply, and taking it as an interface is what
	 * lets one bus serve neoforge and forge.
	 */
	public void useFallback(PayloadFallback fallback) {
	...
	public boolean send(ServerPlayer target, PluginMessageChannel channel, byte[] payload) {
		...
		PayloadFallback fallback = payloadFallback;
	...
	private AmqpTransport createAmqpTransport() {
		try {
			return new RabbitMqAmqpTransport(this, backendIdentity, logger.scope("Amqp/rabbitmq"), loggingEnabled);
		} catch (RuntimeException exception) {
			logger.error("Không thể khởi tạo AMQP transport RabbitMQ. Dùng no-op transport.", exception);
			return new NoopAmqpTransport();
		}
	}
}
